...
def LowerControl(parameters, X0, dt, q):  # size 28x3
    ...
    dRa = 1 * Epos[:, 0] + 1 * ErfxA  # size: 4x1
    dRh = 1 * Epos[:, 1] + 1 * Erfxh  # size: 4x1
    dRk = 1 * Epos[:, 2] + 1 * ErfxK  # size: 4x1
    ...
    f = open('TaoA.txt', 'a')  # size: 3x1
    f.write(str(TaoA[0]) + ',' + str(TaoA[1]) + ',' + str(TaoA[2]) + ',' + str(TaoA[3]) + '\n')
    f.close()
    f = open('TaoH.txt', 'a')  # size: 3x1
    f.write(str(TaoH[0]) + ',' + str(TaoH[1]) + ',' + str(TaoH[2]) + ',' + str(TaoH[3]) + '\n')
    f.close()
    f = open('TaoK.txt', 'a')  # size: 3x1
    f.write(str(TaoK[0]) + ',' + str(TaoK[1]) + ',' + str(TaoK[2]) + ',' + str(TaoK[3]) + '\n')
    f.close()
...
def MidControl(X0, q, dq, F, C, Acc, PHI, dt, Gb, Gl, omega, vs, vf, theta, Phi, w, Phihk, qc, Cq, T):
    ...
    X_data_2 = 1 * np.loadtxt('X_data_2.txt')
    Y_data_2 = np.loadtxt('Y_data_2.txt')
    dX_data_2 = 1 * np.loadtxt('dX_data_2.txt')
    dY_data_2 = np.loadtxt('dY_data_2.txt')
    ...
    [roll_ground, pitch_ground] = slope_estimator(Fc, rl)
    phi_grd = 1 * np.array([roll_ground, pitch_ground, 0])
    Rgrd = Rotation_matrix(roll_ground, 0, pitch_ground)
    rb_grd = body_orientation(Gb, phi_grd)
    drb = rb - rb_grd
    wb = np.zeros([3, 4])
    drb[0, :] = np.zeros([1, 4])
    for i in range(0, 4):
        if F[i] > - 0.01 * F_threshold:
            J = Jacobian_calculation(q, Gl,i)
            ...
            Ji = inv(Jn)
            wb[:, i] = -1 * np.dot(Ji, drb[:, i])
            if abs(wb[1, i]) > 5:
                wb[1, i] = wb[1, i] * 5 /abs(wb[1, i])

    f = open('drb.txt', 'a')  # size: 3x1
    f.write(str(rb[0, 0]) + ',' + str(rb[0, 1]) + ',' + str(rb[0, 2]) + ',' + str(rb[0, 3]) + ',' + str(rb[1, 0]) + ',' + str(rb[1, 1]) + ',' + str(rb[1, 2]) + ',' + str(rb[1, 3]) \
            + ',' + str(rb[2, 0]) + ',' + str(rb[2, 1]) + ',' + str(rb[2, 2]) + ',' + str(rb[2, 3]) + '\n')
    f.close()

    f = open('wb.txt', 'a')  # size: 3x1
    f.write(str(wb[1,0]) + ',' + str(wb[1,1]) + ',' + str(wb[1,2]) + ',' + str(wb[1,3]) + '\n')
    f.close()
    ...
    if abs(vf) < 0.005:
        ...
        PhiT = np.array([2, 0, 2, 0]) * PI / 2  # size: 4x1
    ...
    if abs(vf) >= 0.1:
        ...
        PhiT = np.array([0, 0.5, 2.5, 2]) * PI / 2  # size: 4x1
    ...
    for i in range(0, 4):
        ...
        if F[i] < 0.1 * F_threshold:
            xv = 1.5 * Regression_GP(Kt, Cq, X_data_2)
            yv = Regression_GP(Kt, Cq, Y_data_2)

        else:
            xv = 1 * Regression_GP(Kt, Cq, X_data_2)
            yv = 0 #* Regression_GP(Kt, Cq, Y_data_2)
    ...
    for i in range(0, 4):
        ...
        rt[:, i] = np.array([-0.0, -0.225, 0]) - fi * drb[:, i] + fv[:, i] + rb[:, i]
        rtb[:, i] = np.array([-0.0, -0.225, 0]) - fi * drb[:, i] + rb[:, i]
    ...
    for i in range(0, 4):
        ...
        if F[i] < -0.01 * F_threshold:
            ...
            Epos[:, i] = 0 * wb[:, i] + 1.5 * (qPC + alphaAA[:, i] - q[:, i])

    f = open('qPC.txt', 'a')  # size: 4x1
    f.write(str(qPC[0] + alphaAA[0, 3]) + ',' + str(qPC[1]) + ','
            + str(qPC[2]) + '\n')
    f.close()
    # =============================Reflex generator===========================
    # -----------------------------------=-=-----------------------------------
